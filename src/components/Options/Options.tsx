export const Options = ({ questionData, answer, dispatch }: any) => {
  ...
  const DefaultStyle =
    'block cursor-pointer w-full text-white border-slate-700 border-2 py-2 px-5  mb-2 rounded-full text-left disabled:cursor-not-allowed';
      ...
      {options.map((option: any, index: number) => {
        return (
          <button
            key={index}
            type='button'
            className={` ${DefaultStyle} ${isDisabled ? '' : DisabledStyle} ${
